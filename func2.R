...
for (i in 4:n) {
   ...
   #lmfit_bc<-lm(y ~ T0 + pscore,family = binomial(link = "probit"))
    ...
    #result[i] <- coefficients(lmfit_bc)[2]
    result[i]<-summary(lmfit_bc)$coefficients[8]
    result <- abs(result)
	...
	# result_orig[i] <- coefficients(lmfit_org)[2]
	ts=ncol(X2)
	result_orig[i]<-summary(lmfit_org)$coefficients[3*ts+2]

    result_orig <- abs(result_orig)
    ## IVD model T=X[,1]; pmodel_IVD=lm(T~C); pcoeff_IVD=coefficients(pmodel_IVD); pscore_IVD=
	...
	#tempresult[j] <- coefficients(lmfit_clust)[2]
	ts=ncol(subdataX2)
	tempresult[j]<-summary(lmfit_clust)$coefficients[3*ts+2]
...
#lmfit_clust<-lm(subdataY~subdataX2,family = binomial(link = "probit"))
	...
	#tempresult[j] <- coefficients(lmfit_clust)[1]
	tempresult[j]<-summary(lmfit_clust)$coefficients[3]
...
## cluster the data based on pscore
