...
for (i in 1:n) {
	y_raw = read.table("C:/Users/zhuofu/workspace/apacheCommonMath2.1/TestEigenDecomposition/Data9/out.txt")
...
for(j in 1:length(y))
{
  subindex=which(x_cover==y_raw[j,1])
  if (length(subindex)==0){
   ...
   xp[j]=j;
	...
	yp=rep(0,length(x_cover));
 for (j in 1: length(x_cover)){
	subindex=which(y_raw[,1]==x_cover[j])
		if (length(subindex)==0){
		yp[j]=-1;
		}
		else{
...
if (length(which(xp==-1))!=0)
{  
y=y[-which(xp==-1)]
y_raw=y_raw[-which(xp==-1),]
}
y_index=data.matrix(y_raw[,1])
if (length(which(yp==-1))!=0)
{data_raw=data_raw[-which(yp==-1),]}
row=length(y)
